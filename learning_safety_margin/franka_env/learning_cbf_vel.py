...
def vel_learning(user_number):
    ...
    for i in range(0, nSafe):
        fname = oa_dir + 'safe/' + str(i + 1) + '_eePosition.txt'
    ...
    for i in range(0, nUnsafe):
        fname = oa_dir + 'unsafe/' + str(i + 1) + '_eePosition.txt'
        pos = onp.loadtxt(fname, delimiter=',').T
    ...
    for i in range(0, nDaring):
        fname = oa_dir + 'daring/' + str(i + 1) + '_eePosition.txt'
        pos = onp.loadtxt(fname, delimiter=',').T
    ...
    daring_veltraj = []
    for i in range(0, nDaring):
        fname = oa_dir + 'daring/' + str(i + 1) + '_eeVelocity.txt'
    ...
    if save: pickle.dump(data, open( data_dir+"vel_data_dict.p", "wb" ) )
