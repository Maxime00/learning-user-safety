...
data_path = path + '/figures/vel_lim/'
print(data_path)
...
x = np.array([0.,0.,0.,0.,0.,0.])#onp.random.uniform(low=[0.6, 0.2], high=[1.,0.8], size=(3,))
print(bias_param)
...
class DoubleIntegrator():
    ...
    def forward_sim(self, x, u, steps=1):
        return x + np.diag(np.array([x[3], x[4], x[5], u[0], u[1], u[2]])) * self.dt*steps
...
class CBFMPC_Controller(DoubleIntegrator):
    ...
    def __init__(self, centers, stds, theta, bias, dt=0.1, n_steps=10):
        ...
        print(self.n_states, self.n_controls)
        ...
        self.lb_vars = np.hstack((self.lbX, self.lbU))
        self.ub_vars = np.hstack((self.ubX, self.ubU))
        print(self.lb_vars, self.ub_vars)
        self.cost_fn = 0
        ...
        # Set up NLP Problem
        self.nlp = {}
        print('X shape: ', self.X.reshape((-1,1)).shape)
...
if __name__ == "__main__": 
    ...
    xgoal = [0.5,0.4,0.1, 0.,0.,0.]
    xlist = [0.5]#np.linspace(0.4,0.6,3)
    ylist = [-0.3]#np.linspace(-0.4,-0.2,3)
    zlist = [0.1]#np.linspace(0,0.3,5)
    traj = []
    for i in range(len(xlist)): 
        for j in range(len(ylist)):
            x0 = [xlist[i],ylist[j], 0.1, 0.,0.,0.]
